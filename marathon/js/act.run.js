...
var game = new Phaser.Game(_Config.view.width, _Config.view.height, Phaser.CANVAS, 'main_wrap');
game.States = {};
...
game.States.play = function() {
	...
	this.create = function() {
		...
		var btnLeft = game.add.button(game.world.centerX - 162 - 50, 1138 -150 , "button", function() {
			_.throttle(this.plusSpeed, 3000)
		}, this, 0, 0, 0);
		var btnRight = game.add.button(game.world.centerX + 50, 1138 -150 , "button", _.throttle(this.plusSpeed, 3000), this, 1, 1, 1);
		...
		//player
		this._buddy = game.add.sprite(0, game.height - _Con_sprite.buddy.height -(_Con_sprite.ground.height * 2 / 3), "buddy");
...
game.States.over = function() {
	this.create = function() {
		...
		var url = 'https://github.com/channingbreeze/games',
			random = Date.now(),
			token = md5(countTime + score + random +"360kids")
			search = "time=" + countTime + "&score=" + score + "&random=" + random + "&token=" + token;
